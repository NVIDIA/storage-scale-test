...
PYTHON_SCRIPT="${SCRIPT_DIR}/extract-mdtest-elbencho.py"

python_path=$(setup_python_venv) || exit 1

echo "Running mdtest-elbencho analysis..." >&2
echo "$python_path" "$PYTHON_SCRIPT" "$@" >&2
"$python_path" "$PYTHON_SCRIPT" "$@" || error_exit "Failed to analyze mdtest-elbencho results"
